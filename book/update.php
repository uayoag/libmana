...
if (isset($_GET['book_id'])) {
    ...
    function getBookDetails($conn, $bookID)
    {
        ...
        // Check for errors
        if (!$result) {
            die("Error getting book details: " . $conn->error);
        }
...
<div class="wrap">
    <div class="submitBox">
        ...
        <div>
            ...
            <input type="number" name="year" value="<?php echo $bookDetails['book_year']; ?>"><br>
        </div>
            ...
            <select name="authorIDs[]" multiple required>
                ...
                if ($listOfAuthor->num_rows > 0) {
                    while ($author = $listOfAuthor->fetch_assoc()) {
                        $selected = in_array($author['authorIDs'], explode(',', $bookDetails['authorIDs'])) ? 'selected' : '';
                        echo "<option value='{$author['authorIDs']}' $selected>{$author['au_name']}</option>";
                    }
                }
                ?>
            </select>
            ...
            <select name="categoryIDs[]" multiple required>
                ...
                if ($listOfCategories->num_rows > 0) {
                    while ($category = $listOfCategories->fetch_assoc()) {
                        ...
                        $selected = in_array($category['categoryIDs'], $selectedCategories) ? 'selected' : '';
                        echo "<option value='{$category['categoryIDs']}' $selected>{$category['cat_name']}</option>";
                    }
                }
                ?>
            </select>
            ...
            <select name="storageID" required>
                ...
                if ($listOfStorage->num_rows > 0) {
                    while ($storage = $listOfStorage->fetch_assoc()) {
                        $selected = ($storage['storageID'] == $bookDetails['storageID']) ? 'selected' : '';
                        echo "<option value='{$storage['storageID']}' $selected>{$storage['sto_shelf']}</option>";
                    }
                }
                ?>
            </select>
